...
import com.pubutech.blog.business.enums.UserNotificationEnum;
import com.pubutech.blog.business.enums.UserPrivacyEnum;
import com.pubutech.blog.business.enums.UserStatusEnum;
...
import com.pubutech.blog.persistence.mapper.SysUserMapper;
...
public class SysUserServiceImpl implements SysUserService {
    ...
    public boolean update(User user) {
        ...
        if (!StringUtils.isEmpty(user.getPassword())) {
            try {
                ...
                throw new SelfBaseException("密码加密失败");
            }
        }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean updateSelective(UserEntity userEntity) {
        Assert.notNull(userEntity, "User不可为空！");
        SysUser user = BeanConvertUtil.doConvert(userEntity,SysUser.class);
        user.setUpdateTime(new Date());
        if (!StringUtils.isEmpty(user.getPassword())) {
            try {
                user.setPassword(PasswordUtil.encrypt(user.getPassword(), user.getUsername()));
     ...
     */
    @Override
    public PageInfo<User> findPageBreakByCondition(UserSqlCondition condition) {
        PageHelper.startPage(condition.getPageNumber(), condition.getPageSize());
        List<SysUser> sysUsers = sysUserMapper.findPageBreakByCondition(condition);
        if (CollectionUtils.isEmpty(sysUsers)) {
            return null;
        }
        List<User> users = new ArrayList<>();
    ...
    @Override
    public PageInfo<UserEntity> pageUserInfo(SimplePageSqlCondition condition) {
        PageHelper.startPage(condition.getPageNumber(), condition.getPageSize());
        List<SysUser> sysUsers = sysUserMapper.selectAll();
        if (org.springframework.util.CollectionUtils.isEmpty(sysUsers)) {
            return null;
        }
        List<UserEntity> users = new ArrayList<>();
        for (SysUser u : sysUsers) {
            users.add(BeanConvertUtil.doConvert(u, UserEntity.class));
        }
        PageInfo bean = new PageInfo<UserEntity>(users);
        bean.setList(users);
        return bean;
    }
