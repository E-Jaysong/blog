...
import com.pubutech.blog.persistence.mapper.SysRoleMapper;
...
public class SysRoleServiceImpl implements SysRoleService {
     ...
     */
    @Override
    public PageInfo<Role> findPageBreakByCondition(RoleSqlCondition condition) {
        PageHelper.startPage(condition.getPageNumber(), condition.getPageSize());
        List<SysRole> sysRoles = roleMapper.findPageBreakByCondition(condition);
    ...
    @Override
    public PageInfo<RoleEntity> pageRoleInfo(SimplePageSqlCondition condition){
        PageHelper.startPage(condition.getPageNumber(), condition.getPageSize());
        List<SysRole> sysRoles = roleMapper.listAll();
        if (CollectionUtils.isEmpty(sysRoles)) {
            return null;
        }
        List<RoleEntity> roles = new ArrayList<>();
        for (SysRole r : sysRoles) {
            roles.add(BeanConvertUtil.doConvert(r, RoleEntity.class));
        }
        PageInfo bean = new PageInfo<RoleEntity>(roles);
        bean.setList(roles);
        return bean;
    }
